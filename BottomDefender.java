...
public class BottomDefender extends Player {
	...
	public BottomDefender(int x, int y, double a, Color tc, int s) {
		super(x, y, a, tc, s);
	}

	public BottomDefender(Color tc, int s) {
		super(tc, s);
		playerSpeed = DEFENDER_SPEED;
	}

	@Override
	public void goToInitialPosition(int width, int height) {
		setCenter((int) (20 + ((width - 40)) / 5 + side * (width - 40) * 0.6), 20 + (3 * (height - 40) / 4));
